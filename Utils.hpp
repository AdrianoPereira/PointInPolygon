...
// Função auxiliar para formatar as linhas lidas do arquivo
std::vector<double> split(std::string str) {
    std::istringstream iss(str);
    std::vector<std::string> sans(std::istream_iterator<std::string>{iss}, 
...
// Função auxiliar par criar ponto
Point createPoint(std::vector<double> points) {
    Point ans(points[0], points[1]);
    return ans;
}

// Função auxiliar para criar polígono
Polygon createPolygon(std::vector<double> points) {
    std::vector<Point> pts;

    // As coordenadas x e y são recebidas em um único vetor, 
    // uma seguida da outra, este laço cria os pontos para depois das 
    // pares de coordenadas para formar o polígono
...
// Função auxiliar para fazer leitura dos arquivos
...
// Função auxiliar para formatar os nomes das cidades, retirando a extensão .txt
